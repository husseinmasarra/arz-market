...
async function initializeDatabasePostgres() {
  try {
    ...
    // Permanently clean up external demo products and demo categories to keep store strictly DR PHONE
    try {
      await pgPool.query("DELETE FROM products WHERE id < 38 OR category_id <= 16 OR merchant_id IS NULL");
      await pgPool.query("DELETE FROM categories WHERE id <= 16");
      console.log('[Database] Cleaned up all external demo products and categories from PostgreSQL.');
    } catch (e) {
      console.error('[Database] Cleanup note:', e.message);
    }
  } catch (err) {
  ...
  db.serialize(() => {
    ...
    db.get('SELECT COUNT(*) as count FROM categories', [], (err, row) => {
      if (row && parseInt(row.count) === 0) {
        ...
        db.run("DELETE FROM products WHERE id < 38 OR category_id <= 16 OR merchant_id IS NULL", [], () => {});
        db.run("DELETE FROM categories WHERE id <= 16", [], () => {});
      }
    });
  });
}
